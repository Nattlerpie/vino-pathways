...
const FranceConvergenceDashboard = () => {
  ...
  const vulnerabilityIndicators = [{
    ...
    indicator: "Key Market Share",
    value: 0.45,
    type: "dependency",
    risk: t('medium')
  }, {
    indicator: "Seasonal Concentration",
    value: 0.72,
    type: "concentration",
    risk: t('high')
  }, {
    indicator: "Peak Season Dependency",
    value: 0.58,
    type: "dependency",
    risk: t('medium')
  }, {
    indicator: "Transport Mode Concentration",
    value: 0.52,
    type: "concentration",
    risk: t('medium')
  }, {
    indicator: "Primary Mode Dependency",
    value: 0.48,
    type: "dependency",
    risk: t('medium')
  }];
  ...
  const partnershipMatrix = [{
    ...
    sector2: "Aviation",
    opportunity: 78,
    vulnerability_risk: 0.65,
    sustainability: 0.45
  }, {
    sector1: t('gastronomy'),
    sector2: "Cycling",
    opportunity: 88,
    vulnerability_risk: 0.28,
    sustainability: 0.90
  }, {
    sector1: t('cultural'),
    sector2: "Rail",
    opportunity: 82,
    vulnerability_risk: 0.32,
    sustainability: 0.80
  }, {
    sector1: t('religious'),
    sector2: "Rail",
    opportunity: 65,
    vulnerability_risk: 0.45,
    sustainability: 0.75
  }];
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-end mb-4">
          <div className="bg-white rounded-lg shadow-md p-2 flex items-center gap-2">
            ...
            <select value={language} onChange={e => setLanguage(e.target.value as Language)} className="border-none bg-transparent text-sm font-medium focus:outline-none cursor-pointer">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-xl shadow-lg p-8 text-white text-center mb-6">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <button onClick={() => window.open('mailto:your.email@domain.com?subject=Partnership Analysis Discussion')} className="bg-white text-purple-600 px-6 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
              Request Full Data Integration Proposal
            </button>
            <button onClick={() => window.open('mailto:your.email@domain.com?subject=Vulnerability Assessment Methodology')} className="bg-purple-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-purple-400 transition-colors">
              Vulnerability Assessment Methodology
            </button>
            <button onClick={() => window.open('mailto:your.email@domain.com?subject=Regional Partnership Consulting')} className="bg-blue-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-400 transition-colors">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={selectedRegion} onChange={e => setSelectedRegion(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md">
                {Object.entries(regions).map(([key, region]) => <option key={key} value={key}>
                    {region.name}
                  </option>)}
              </select>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => setSelectedView('convergence')} className={`p-2 rounded-md text-sm transition-colors ${selectedView === 'convergence' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                  {t('convergence')}
                </button>
                <button onClick={() => setSelectedView('seasonality')} className={`p-2 rounded-md text-sm transition-colors ${selectedView === 'seasonality' ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                  Saisonnalité
                </button>
                <button onClick={() => setSelectedView('vulnerability')} className={`p-2 rounded-md text-sm transition-colors ${selectedView === 'vulnerability' ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                  {t('vulnerability')}
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <select value={vulnerabilityType} onChange={e => setVulnerabilityType(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md">
        ...
        {selectedView === 'convergence' ? (/* Convergence Analysis */
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart>
                    ...
                    <XAxis dataKey="vulnerability" type="number" domain={[0, 1]} name="Vulnerability Risk" label={{
                  value: 'Vulnerability Risk →',
                  position: 'insideBottom',
                  offset: -5
                }} />
                    <YAxis dataKey="partnerships" type="number" domain={[0, 100]} name="Partnership Potential" label={{
                  value: 'Partnership Potential ↑',
                  angle: -90,
                  position: 'insideLeft'
                }} />
                    <Tooltip formatter={(value, name) => [name === 'vulnerability' ? `Risk: ${(Number(value) * 100).toFixed(0)}%` : `Opportunity: ${value}%`, name === 'vulnerability' ? 'Vulnerability' : 'Partnership Potential']} labelFormatter={label => `Transport Mode`} />
                    <Scatter data={transportModes} fill="#3b82f6" />
                  </ScatterChart>
                </ResponsiveContainer>
              </div>
              <div className="grid grid-cols-2 gap-2 mt-4">
                {transportModes.map((mode, index) => <div key={index} className="flex items-center gap-2 text-sm">
                    <span>{mode.icon}</span>
                    <span>{mode.name}</span>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                {tourismTypes.map((sector, index) => <div key={index} className="border rounded-lg p-3">
          ...
          </div>) : selectedView === 'seasonality' ? (/* Seasonality Analysis */
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            {/* Seasonal Patterns */}
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-4">
                {Object.entries(regions[selectedRegion].vulnerability).map(([type, score]) => <div key={type}>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                {vulnerabilityIndicators.map((indicator, index) => <div key={index} className="border-l-4 pl-3" style={{
              borderColor: getVulnerabilityColor(indicator.value)
            }}>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                {partnershipMatrix.slice(0, 5).map((partnership, index) => <div key={index} className="border rounded-lg p-3">
                    <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <span className={`font-medium`} style={{
                    color: getVulnerabilityColor(partnership.vulnerability_risk)
                  }}>
        ...
        <div className="text-center mt-8 text-sm text-gray-500">
          <p>Strategic analysis by Natalie Gillespie - Bridging academic research with business strategy</p>
